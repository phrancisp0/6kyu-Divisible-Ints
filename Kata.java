public class Kata
{
  ...
  {
    int total = 0;
    String bigString = new Integer(n).toString();
    for (int s=1; s<bigString.length(); s++) {
      for (int i=0; i<=bigString.length()-s; i++) {
        String smallString = bigString.substring(i,i+s);
        Integer smallInt = new Integer(smallString);
        if (smallInt != 0 && n % smallInt == 0) {
          total += 1;
        }
      }
    }
